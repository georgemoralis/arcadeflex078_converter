...
public class carpolo
{
	...
	//									   5.7940, 6.1608, 6.5436, 6.9620 };
		...
		for (i = 0; i < Machine.drv.total_colors - 1; i++)
		{
			...
			r = ((r_voltage[color_prom.read()>> 5         ] - MIN_VOLTAGE) / (MAX_VOLTAGE - MIN_VOLTAGE)) * 255.;
			/* green component */
			bit0 = (color_prom.read()>> 4) & 0x01;
			bit1 = (color_prom.read()>> 3) & 0x01;
			bit2 = (color_prom.read()>> 2) & 0x01;
			//g = 0x21 * bit0 + 0x47 * bit1 + 0x97 * bit2;
			g = ((g_voltage[(color_prom.read()>> 2) & 0x07] - MIN_VOLTAGE) / (MAX_VOLTAGE - MIN_VOLTAGE)) * 255.;
			/* blue component */
			bit0 = (color_prom.read()>> 1) & 0x01;
			bit1 = (color_prom.read()>> 0) & 0x01;
			//b = 0x4f * bit0 + 0xa8 * bit1;
			b = ((b_voltage[color_prom.read()& 0x03       ] - MIN_VOLTAGE) / (MAX_VOLTAGE - MIN_VOLTAGE)) * 255.;
	...
	static int check_sprite_sprite_collision(int x1, int y1, int code1, int flipy1,
	...
	static int check_sprite_left_goal_collision(int x1, int y1, int code1, int flipy1, int goalpost_only)
	{
		int collided = 0;
	...
	static int check_sprite_right_goal_collision(int x1, int y1, int code1, int flipy1, int goalpost_only)
	{
		int collided = 0;
